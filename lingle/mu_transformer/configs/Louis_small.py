...
def get_base_config():
    ...
    config.n_layer = 2  # depth, should stay const for mu-transfer
    config.d_model = 32  # current model width
    config.d_base = 32  # base model width for relative scaling rules
    config.d_head = 32 
    ...
    config.qk_scale = 1 / config.d_head
    ...
    config.tokens_per_global_batch = (
        16 * config.sequence_len  # batch size of 16 kept constant
    ...
    config.n_print_step = 50  # print every
    config.n_save_step = 100000  # checkpoint every <-- not necessary for small models
    config.n_eval_step = 1000  # eval steps per checkpoint
    config.n_warmup_step = (
        2500  # warmup steps during pretraining - will be set later manually
    )
